...
# TODO Lambda klicove slovo pro vytvareni funkci na jeden radek

def is_even(x):
    return x % 2 == 0


print(list(filter(is_even, [-1, -2, 0, 5, 0, 0, 2])))
# zapis pres lambdu
print(list(filter(lambda x: x % 2 == 0, [-1, -2, 0, 5, 0, 0, 2])))
print((lambda x: x)('hi'))

(lambda x: print(f'{x} Hii'))(4)


array = [(1, 4), (2, 0), (3, 3), (2, 3)]

print(sorted(array, key=lambda tup: (tup[1], tup[0])))
                        # lambda urcim si podle ktereho prvku seradime dvojice ( v tomto pridade pokud je tup[1] stejny
                        # porovnej podle tup[0]
# Task 1
